...
const nextConfig = {
  ...
  // Security headers for local and Docker dev parity.
  // Vercel production uses vercel.json; Next.js headers() covers every other
  // environment (local `next dev`, Docker `next start`, standalone runner).
  // Keep values in sync with /vercel.json at the repo root.
  async headers() {
    return [
      {
        source: "/(.*)",
